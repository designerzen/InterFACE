...
export const base64ToBinary = input => {
...
// NB. This method *can* be used inside a worker :)
export const base64Decode = base64 => { 
	return convertBinaryToUInt8Array(base64ToBinary(base64))
}

// If ATOB is still available, probably best to use that...
export const base64DecodeNative = (base64) => {
	return convertBinaryToUInt8Array(atob(base64))
}
 ...
 * @returns {Boolean} whether this is probably a base 64 file
 */
export const isBase64 = (stringData) => {
	...
	// check to see if it ends in a =, if so it is probably base 64
	...
	if (!/^[A-Za-z0-9+/]+={0,2}$/.test(stringData)) 
	{
		return false
	}
