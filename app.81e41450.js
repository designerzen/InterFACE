...
},{"./index":"b6SeL"}],"b6SeL":[function(require,module,exports) {
/**
...
const DOMAIN = (0, _locationHandler.getEditionFromURL)()[0];
const HOST = (0, _locationHandler.getRefererHostname)();
...
if (debugMode) console.log("TEST : Initialising", {
    capabilities,
    DOMAIN,
    HOST,
    LTD
...
const start = ()=>{
    (0, _loadProgress.setLoadProgress)(0.5, " ");
    ...
    const dominOptions = (0, _settings.getDomainDefaults)(HOST);
    ...
    require("9e1a49fd58bb02e7").then(async ({ createInterface })=>{
        ...
        try {
            const application = await createInterface(defaultOptions, store, capabilities, language, (loadProgress, message, hideLoader = false)=>{
                if (failed) // already failed so why show more loading?
                return;
                if (loadProgress !== 0.5) // reset the timer during the half time show
                ...
                // else 
                if (hideLoader) {
                    (0, _loadProgress.setLoadProgress)(loadProgress, "", true);
                    setTitle(title);
                    ...
                    (0, _loadProgress.setLoadProgress)(1, "Ready!", true);
                    setTitle(title);
                }
            });
            ...
            if (debugMode) console.log(`InterFACE Version ${(0, _version.VERSION)} from ${(0, _locationHandler.getReferer)()} in ${language} used ${application.count} times, last time was ${Math.ceil(application.timeElapsedSinceLastPlay / 1000)} seconds ago`, {
                application,
                defaultOptions,
                referer: (0, _locationHandler.getReferer)()
            });
        } catch (error) {
            // body.classList.add("failed")
            //uninstall()
            (0, _errors.showError)(error, "Oh no! Try a hard refresh (CTRL-SHIFT-R)", true);
...
const checkPWAUpdates = ()=>{
    ...
    return (0, _pwa.installOrUpdate)(debugMode, runningVersion).then((state)=>{
        // this is the amount of time to run before we "check" for things
        // const TIME_BEFORE_REFRESH = 24 * 60 * 60 * 1000
        if (debugMode) {
            console.log("TEST : Fetched install status", {
                state,
                debugMode,
                runningVersion
            });
            console.info("PWA", state.log, {
                state
            });
        }
 ...
 */ const checkPlatformUpdates = async ()=>{
    ...
    else if (capabilities.pwa) // we are running this as a PWA!
    await checkPWAUpdates();
    else if (capabilities.pwaPossible) // potential for it to be a webapp
    await checkPWAUpdates();
    return start();
};
...
checkPlatformUpdates().finally(()=>{
    if (debugMode) console.log("Starting PhotoSynth v." + runningVersion);
});
...
},{"./location-handler":"iYStN","./dom/load-progress":"7KwQi","./version":"7AyKa","./i18n":"gseYz","./settings":"heiex","./pwa/pwa":"kOXsT","./store":"6qVfF","./dom/errors":"kOYvE","./dom/tooltips":"8RoHD","./hardware/mouse":"5goDw","./capabilities":"4LUzi","./attractor":"kFspz","9e1a49fd58bb02e7":"14bXE","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"iYStN":[function(require,module,exports) {
/**
...
parcelHelpers.export(exports, "getEditionFromURL", ()=>getEditionFromURL);
...
const setLoadProgress = (progress, message, complete = false)=>{
    ...
    if (loadMeter < 0) loadStartedAt = now;
    const timeElapsed = now - loadStartedAt;
    const timeAsProgress = timeElapsed / TOTAL_LOAD_DURATION;
    if (hasProgressed) // animate this...
    loadMeter = loadMeter < rounded ? loadMeter + RATE : loadMeter > rounded ? loadMeter - RATE : loadMeter;
    ...
    if (complete && !hasProgressed) progressBar.parentNode.setAttribute("data-loaded", true);
    else progressBar.parentNode.setAttribute("data-loaded", false);
    // check to see if the message has changed
    if (rounded === 0) text = LOAD_MESSAGES[0];
    ...
    } else if (rounded === 100) text = LOAD_MESSAGES[LOAD_MESSAGES.length - 1];
    else {
    ...
    if (hasProgressed) requestFrame = requestAnimationFrame(()=>{
...
parcelHelpers.export(exports, "DEFAULT_TASKS_VISION_OPTIONS", ()=>DEFAULT_TASKS_VISION_OPTIONS);
...
parcelHelpers.export(exports, "DEFAULT_VOICE_OPTIONS", ()=>DEFAULT_VOICE_OPTIONS);
...
const DEFAULT_TASKS_VISION_OPTIONS = {
...
var _servicewaitingPolyfill = require("./servicewaiting.polyfill");
var _version = require("../version");
...
const isRunningAsApp = (0, _platform.isInWebAppiOS) || isInWebAppChrome || (0, _platform.isTWAAndroid) || (0, _platform.isMicrosoftStore) || false;
...
const platform = {
    ios: (0, _platform.isIOS),
    android: (0, _platform.isTWAAndroid),
    microsoft: (0, _platform.isMicrosoftStore),
    firefox: (0, _platform.isFirefox)(),
    pwa: isRunningAsApp,
    offline: !isOnline,
    displayMode
};
...
const showChangelog = async (domElement)=>{
    const { injectChangeLog, fetchChangesAsText } = await require("65185964d0d8965d");
...
const installOrUpdate = async (debug = false, currentlyRunningVersion = "")=>{
    ...
    if (isRunningAsApp) log.push(`PWA ${(0, _version.VERSION)} Installed, checking for updates`, {
        deferredPrompt,
        isFirstRun,
        isInWebAppiOS: (0, _platform.isInWebAppiOS),
        isInWebAppChrome
    });
    else {
    ...
    // check if installing...
    if (registration?.installing) // if the feature exists, we can work out how heavy the app is
    {
    ...
    if (registration) {
        ...
        registration.addEventListener("updatefound", async ()=>{
            ...
            if (installingWorker?.waiting && navigator.serviceWorker.controller) // FIXME: 
    ...
    if (!serviceWorker) {
        serviceWorker = await navigator.serviceWorker.register(require("343bdc5f53043b15"));
...
},{"./servicewaiting.polyfill":"1HUlq","../version":"7AyKa","./platform":"kveoH","65185964d0d8965d":"9CbHL","343bdc5f53043b15":"9DOMT","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"1HUlq":[function(require,module,exports) {
// Brilliant prolyfil by dfabulich
// https://github.com/w3c/ServiceWorker/issues/1222#issuecomment-351566460
if (!("waiting" in navigator.serviceWorker)) navigator.serviceWorker.waiting = new Promise(function(resolve) {
...
},{}],"kveoH":[function(require,module,exports) {
// Determine as much functionality as possible
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "isInWebAppiOS", ()=>isInWebAppiOS);
parcelHelpers.export(exports, "isIOS", ()=>isIOS);
parcelHelpers.export(exports, "isTWAAndroid", ()=>isTWAAndroid);
parcelHelpers.export(exports, "isMicrosoftStore", ()=>isMicrosoftStore);
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"9CbHL":[function(require,module,exports) {
module.exports = require("fbf5049eec2d442d")(require("fd2b3640a32b7882").getBundleURL("hAF7M") + "changes.a7179ccd.js" + "?" + Date.now()).catch((err)=>{
    delete module.bundle.cache[module.id];
    throw err;
}).then(()=>module.bundle.root("3MFIV"));

},{"fbf5049eec2d442d":"61B45","fd2b3640a32b7882":"lgJ39"}],"61B45":[function(require,module,exports) {
"use strict";
...
},{}],"9DOMT":[function(require,module,exports) {
module.exports = require("f1046cc9d2bf412b").getBundleURL("hAF7M") + "service-worker.js" + "?" + Date.now();

},{"f1046cc9d2bf412b":"lgJ39"}],"6qVfF":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
const addTooltip = (element)=>{
    const callback = (event)=>{
        ...
        const position = event.target.nodeName === "BUTTON" ? event.target : event.target.parentElement;
...
parcelHelpers.export(exports, "hasTouchEvents", ()=>hasTouchEvents);
parcelHelpers.export(exports, "isTouchDevice", ()=>isTouchDevice);
parcelHelpers.export(exports, "hasFileHandler", ()=>hasFileHandler);
parcelHelpers.export(exports, "isNode", ()=>isNode);
parcelHelpers.export(exports, "isElectron", ()=>isElectron);
var process = require("2b122511d9d7dc8a");
...
},{"2b122511d9d7dc8a":"d5jf4","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"d5jf4":[function(require,module,exports) {
        ...
        if (!userActive) // inactive - ATTRACT MODE
        {
            if (barProgress === 0) {
                ...
                players.forEach((player)=>{
                    if (!player.isFormShowing && player.timeSinceInstrumentChanged > DURATION_BEFORE_AUTOMATIC_INSTRUMENT_CHANGE) //console.log("Has player been stuck on this instrument too long?", player.timeSinceInstrumentChanged, DURATION_BEFORE_AUTOMATIC_INSTRUMENT_CHANGE)
                ...
                if (this.barCounter % 2) this.application.setRandomDrumPattern();
            }
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"14bXE":[function(require,module,exports) {
module.exports = Promise.all([
    require("c95f843ad684a9c8")(require("7bd9e18898db3680").getBundleURL("hAF7M") + "interface.314bad26.js" + "?" + Date.now()).catch((err)=>{
        delete module.bundle.cache[module.id];
        throw err;
    }),
    require("c95f843ad684a9c8")(require("7bd9e18898db3680").getBundleURL("hAF7M") + "interface.6185bf9f.js" + "?" + Date.now()).catch((err)=>{
        delete module.bundle.cache[module.id];
        throw err;
    })
]).then(()=>module.bundle.root("eYDPu"));

},{"c95f843ad684a9c8":"61B45","7bd9e18898db3680":"lgJ39"}]},["fE7Fk","due8H"], "due8H", "parcelRequireaaed")

//# sourceMappingURL=app.81e41450.js.map
