...
})({"f86476b6dfe525fe3e35c3b4270bbe11":[function(require,module,exports) {
"use strict";
...
const body = document.documentElement; // const shareMenu = document.getElementById('share-menu')
...
const installer = async (defer = false) => {
  let openModal = false;
  let manifestData;
  let manifestPath = "./manifest.webmanifest";
  let relatedApps = []; // handle iOS specifically
  ...
  const shouldShowInstall = () => {
    const eligibleUser = isSupportingBrowser && relatedApps.length < 1 && (hasPrompt || isIOS);
    return eligibleUser;
  ...
  const createOverlayMarkup = () => `<button id="openButton" class="install-app">
	Install InterFACE Version ${_version.VERSION}
	</button>
	...
	<dialog open>
		...
		<div id="contentContainer">
			...
			${manifestData.features ? `<div id="keyFeatures">
				<h3>Features</h3>
				<ul>
				${manifestData.features ? manifestDatafeatures.map(feature => ` <li>${feature}</li> `) : null}
				</ul>
			</div>
			</div>` : null}

			${manifestData.screenshots ? html`
					<div id="screenshotsContainer">
						<button
							@click="${() => (void 0).scrollToLeft()}"
    ...
    return html`
			...
			<div id="descriptionWrapper">
				<h3>${(void 0).descriptionheader}</h3>
				<p id="manifest-description">${(void 0).manifestdata.description}</p>
			</div>
			
			</div>
		
		<button id="openButton" class="install-app">Install InterFACE Version ${_version.VERSION}</button>
  ...
  const install = async () => new Promise(async (resolve, reject) => {
    if (deferredPrompt) {
      ...
      await cancelInstall(); // brute force hide the button?
  ...
  const begin = async destination => {
    ...
    if (hasPrompt && "standalone" in navigator && navigator.standalone === false || showInstaller && installed === false) {
      ...
      button.setAttribute("aria-label", `Click to install ${manifestData.short_name} V:${_version.VERSION.replaceAll(".", "-")} Date:${_version.DATE}`);
...
//     `;
//   }
},{"./ui":"cc509f6dc4e34d6bd3c8d4b98935925a","./version":"25488683cc04bb43faf629e105172bc8"}]},{},[], null)

//# sourceMappingURL=install.1b8d2fef.js.map
